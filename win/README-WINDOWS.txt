...
-----------------------------------
  ...
  cookies_from_browser  chrome|firefox|edge|vivaldi|brave|opera|... - same thing without
                        ...
                        The in-tab modes direct/replay/cover need NO cookies:
...
лесенки его не снимают, а только усиливают подозрительность. Поэтому с 0.5.3:
  ...
  - record удалён в 0.6.16 (владелец им не пользовался): прежний cfg-mode
    «record» без падения ведёт в resolve.
  ...
  - 0.6.0-0.6.3 (ветка в GitHub-репо): ThrottleQueue - отложенные на лимитах треки
    живут в .ytm-throttle-queue.json рядом с архивом и докачиваются сами (интервал
    вместо дедлайна, sync с архивом, потолок 6 попыток, монитор раз в час); плейлист
    добивает ВСЕ упавшие позиции (0.6.2: раньше из шума бралась одна последняя id);
    итог листа считает спасённое по-честному (0.6.3); browser-очередь не теряет режим
    при ids из browse-фолбэка; панель ждёт ytcfg перед /browse.
  - 0.6.4: «очередь со страницы» читает панель «Следующее» (id строк очереди, когда
    ссылок нет - раньше это и было «очередь видна, скачать нельзя»); кнопка «стоп»
    в ряду статуса: прерывает очередь/прогон после текущего трека и отменяет ещё не
    начатые задачи (POST /job/<id>/cancel); фоновые retry кнопка не трогает.
  - 0.6.5: липкий клиент - успешный в этой сессии клиент идёт первым и
    у одиночных треков, и у всего листа: альбом перестает сначала гонять
    заведомо мертвые попытки. Ничего не отбрасывается: провал липкого = обычная лестница.
  - 0.6.6: очередь «Следующее» читается из ответа get_queue (в DOM-строках id больше
    нет - 95 строк без единого свойства), конфиг страницы читается через менеджер
    .get()/data_ - чинит browse-фолбэк «плейлиста» и само-запросы /player, которые в
    песочнице Tampermonkey раньше всегда видели пустой ytcfg.
  - 0.6.7: сообщения yt-dlp в cp1251 больше не обнуляют вывод попытки (был ложный
    «лимит аккаунта» из-за одного битого байта); «стоп» переехал в шапку панели -
    виден без скролла; «Понравившиеся» качаются через browse библиотеки самой
    вкладки (FEmusic_liked_videos/songs), YY на LM по-прежнему не подменяется.
  - 0.6.8: строки нового UI (списки без ссылок) собираются прямо из DOM - liked с
    800 строк больше не «пустая страница»; liked-вкладка на /playlist?list=LM берёт id
    из адресной строки; поле «сколько» ограничивает скролл и постановку (первые 5 -
    это 5 строк, а не весь лист); если browse закрыт лимитом - в очередь идут строки,
    которые ты видишь на экране.
  - 0.6.9: 800 в логе был нашим потолком вырезания, а не лимитом YouTube -
    browse читает до 5000 строк, перехват очереди - до 2000; путь «строки
    со страницы» накапливает ленивую ленту скроллом (кап = поле «сколько»).
    Из строки «Исполнитель · Альбом · дата» в job едут title/artist/album - теги
    TPE1/TALB заполняются даже когда web-ответ молчал. Год/жанр не
    выдумываем: источника в строке нет.
  - 0.6.10: починены ТЕКСТОВЫЕ теги mp3 - имена фреймов роняли запись ещё
    с 0.5.x, в свойствах Windows жил только filename (то самое «мало
    метаданных»); теперь TIT2/TPE1/TALB/TDRC/TRCK/TPE2 пишутся, обложка и
    теги проверяются живым read-back. Исполнитель альбома = артист трека,
    когда отдельного не указано; жанр идёт в ©gen (gnre в mutagen только
    читается); год/дорожка/жанр подхватываются из info.json, если yt-dlp их
    принёс - пустые поля остаются пустыми, не выдумываем. Бюджет скролла для
    13k-liked поднят (400->1000 прогонов): 800-куски больше не хвост,
    а byline «Исполнитель · Альбом» с фитой через «и» разбирается ровно как на
    скриншоте.
  - 0.6.11: большие листы докучиваются до конца: настоящий скроллер
    ищется поднятием от строки списка (раньше сбор обрывался на отрисованных
    ~800 из 1884 - это был НЕ лимит YouTube); «MAK DADDY - Bezos» больше не
    едет в поле «Название» - при известном артисте префикс «Артист - » срезается
    и в теге, и в имени файла; «0» в поле proxy (панель и ytm-dl.ini) = качать
    нацело без прокси (TUN-VPN), в ini починены переносы строк и добавлены
    подсказки.
  - 0.6.12: метаданные строк читаются по признаку, а не по позиции:
    хвостовая ячейка «1:25» (длительность) больше не записывается артистом,
    номера-индексы - в anything; без real byline артист не выдумывается.
    «№ 0» в свойствах исчезает: нулевые track_number из info.json в теги не
    пишутся (3/12 остаётся корректной тройкой).
  - 0.6.13: открытая панель «Следующее» больше не мешает сбору: большие
    листы собираются со страницы (1884 строки, а не 100 из панели), а теги
    берутся из строк страницы - альбом перестал въедать поле «исполнитель»
    и префикс имени файла. Список, упавший и у companion, и в browse,
    в этой сессии больше не обивает двери: сбор стартует сразу, без
    «долго думало».
  - 0.6.14: скролл большого листа больше не зависит от угаданного селектора -
    контейнер выбирается пробой (что реально листается, то и крутим; выбор
    виден в ytm-run.log строкой «скроллер: …»). Если «Название» снова начнётся
    с «Артист - », это чинится само: префикс срезается и в панели, и на стороне
    компаньона, NBSP-пробелы YouTube не мешают. Фиты «A и B» в исполнителях
    пишутся как «A, B» - так их видят тегайзеры. И главное: повторное нажатие
    «лист» летящую очередь НЕ останавливает (раньше «само на 11 остановилось»
    было именно повторным нажатием); «стоп» по-прежнему стоп.
  - 0.6.16: прокрутка больше не обгоняет подгрузку YouTube (пауза 260мс, шаг
    85% окна, ожидание спиннера НЕ засчитывается в «конец списка»): боевые
    «долго-долго и 100 строк» были гонкой со выдачей чанков. Мёртвый
    контейнер вычисляется за 14 раундов и меняется на живой; перед сдачей
    сбор прыгает в настоящий низ. На панели видно «собрано N из M».
    Префикс «Артист - » в названии/имени файла добит до конца (артист через
    запятую сверяется со «и»/«&»-склейками заголовка). «Дорожка: 0» в
    свойствах тоже добит (yt-dlp приносит track списком [0], фильтр его
    пропускал). Кнопка record убрана по вашей просьбе - режимов стало
    четыре, все рабочие.
  - 0.6.27: если очередь встала (лимит аккаунта «3 ошибки подряд» или «стоп»), 
    скролл больше не крутится впустую - он сворачивается на следующем раунде, 
    а итог очереди («N готово, стоп на лимите · ждут retry: M») остаётся на 
    панели и не затирается статусом сбора. «Говорит что качает, а новых нет» 
    было именно этой парой: очередь умерла, скролл жил. Повтор после лимита - 
    то же «лист целиком»: скачанное архив пропустит, retry-очередь дойдёт сама.
  - 0.6.26: «лист целиком» качает параллельно скроллу (раньше очередь ждала
    конца всей прокрутки - минуты простоя загрузчика; теперь первая пачка
    стартует сразу, «стоп» = докачать собранное). В лог пишется АТРИБУЦИЯ
    докорма - какой скроллер реально двигал строки. И цикл стал примерно
    вдвое короче: когда рост пойман на упоре в низ, карусель немых
    контейнеров больше не съедает полминуты на каждый чанк.
  - 0.6.25: скролл ОТКАТАН к поведению, которое реально доскрашивало списки
    (0.6.19): полная карусель кандидатов и финальный дожим в низу без
    привязки к числу в шапке - выяснилось, что чанки подгрузки доезжают
    именно пока цикл жив, и мои «ускорения» срезали сам рост (694 -> 398 ->
    100 строк). Полезное осталось: подтверждение конца по ответу YouTube
    (экономит минуту там, где список правда закончился), отсутствие прыжков
    наверх, мгновенная гибель немых контейнеров и параллельная загрузка
    первой пачки (0.6.23).
 «конец списка» из 0.6.23 (бой: сбор
    умирал на 100 строках, не начав скроллить - вердикт выносился по последнему
    browse-ответу страницы ДО клика). Теперь отсутствие continuation только
    ПОДТВЕРЖДАЕТ безмолвие у низа (тогда дожим не нужен и выход мгновенный),
    сигнал обнуляется на старте каждого прогона, а обещание continuation
    действительно 12 секунд. Параллельная загрузка 0.6.23 не тронута.
: первая собранная пачка
    качается сразу, остальной лист скроллится параллельно и докормчивает
    очередь на ходу (кнопка «стоп» глушит оба процесса). А «подождать/сдаться»
    решает не таймер, а сама страница: пока YouTube тянет очередную пачку
    списка - ждём; как только в его же ответе нет continuation - это конец,
    выходим сразу. Боевое «недоскоролил пару секунд и запушил в очередь»
    адресовано именно здесь.
: кандидаты, которых проба уже
    отвергла, больше не прокруываются по 14 раундов со сбросом наверх
    («вернулся вверх страницы» - это оно и было); финальный дожим внизу
    «дышит» (отход на полтора экрана и возврат) - так будится ленивая
    подгрузка YouTube, которая на статичный низ не реагирует. Строки
    вердиктов скролла в ytm-run.log больше не обрезаются на 300 символах.
 (артист и альбом - в тегах;
    «1nonly, Shakewell - WHO GON' SLIDE.mp3» больше не будет). Одноимённые
    треки в одной папке считаются уже скачанными (пропуск, без копий « (1)»).
    Дожим конца списка стал быстрым: набрал 90% от обещанного шапкой -
    хвост из удалённых с площадки треков не ждут (12 раундов вместо 45).
 больше не пишется в программу (Origin-гейт:
    браузер обязан представляться, curl и ytm.bat - нет) и размерные лимиты
    (/media 768 MiB, /log 64 KiB, /job 1 MiB). Пользователю ничего не меняется:
    панель работает как работала.
: у новых строк YouTube Music
    артисты и альбом - отдельные колонки без разделителя в тексте, и скрипт
    ставил АЛЬБОМ на место артиста («ALBUM - Track.mp3»). Теперь строка
    разбирается по колонкам. Прокрутка стала терпеливее: если список рос, а
    шапка обещает больше - 40 раундов тишины это ещё не «конец», плюс финальный
    дожим внизу страницы (боевое «сошлось 791 из 1884 и скачали только их»).
    Номера дорожек в теги больше не пишутся вовсе (просил владелец): позиция в
    плейлисте - не номер альбома. В ytm-run.log видно ВСЕХ кандидатов скролла
    (строку вердиктов резало на четвертом).
    ожидания - и решение отдаётся watchdog'ам); кнопка «стоп» работает и
    во время прокрутки листа (возвращает уже собранное, статус говорит
    «сбор остановлен»); в лог пишутся вердикты ВСЕХ кандидатов скролла -
    какой двигает строки, какой только притворяется (просил владелец:
    «проверка всех возможных и вывод, какой сработал»); статус больше не
    врёт «прокручиваю» до первого скролла. Компаньон в /hello показывает
    sticky_client - на каком клиенте осёл движок.
  - 0.6.17: скроллер теперь выбирается по тому, что нельзя подделать -
    ДВИЖЕНИЕ СТРОК на экране, а не запись чисел в scrollTop. Плюс событиям
    scroll/колесо после каждого программно-листа (виртуализаторы слушают
    их, а не свойство) и окну как последнему кандидату. Боевое «под ручным
    скроллом догружает, под программным нет» - это лечит точно.
  - 0.6.15: «спотыкание о подгрузку» побеждено: пока YouTube догружает список,
    пауза вместо сдачи; если выбранный контейнер упёрся - берётся следующий
    (в логе видно какой). Относительные ссылки новых списков читаются (это был
    «0 строк»). Шапка листа («1 884 трека») задаёт цель: набрал - конец,
    всё отрисовано сразу (альбом, 20-трековик) - старт без companion и browse.
    Кнопка «стоп» - красная, в одном ряду с кнопками скачивания.
...
[ ] 6. Качать
    ...
    Cookies: когда они реально нужны
       • НЕ нужны для обычного трека в режиме direct/replay и для пункта
...
"YouTube недоступен вообще" (РФ): прокси
    ...
    -> почему трек скачался и без proxy=: байты шли ИЗ ВКЛАДКИ (режимы
       direct/replay качает браузер, а браузер у тебя проксируется). Как только
    ...
    Как убедиться, что версия новая: заголовок панели сам показывает v0.6.27 (с
    этой версии цифра берётся из @version, а не захардкожена), а статус «companion
    ok (сборка 0.6.27)» - версию компаньона; при разнобое панель пишет «версии
    разошлись». А надёжнее всего: Tampermonkey -> свойства скрипта -> 0.6.27 (если там
