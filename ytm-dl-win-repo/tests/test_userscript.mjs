...
group('0.5.2: mediaFmt/sniffedFull, зеркало в /log, поля панели');
{
  ...
  ok(/_mirrorSeen\.get\(key\) \|\| 0\) < 2000/.test(src), 'дедуп 2 секунды - хуки не забьют лог');
    ok(/@version      0\.6\.17/.test(src), 'шапка 0.6.17');
  ...
  ok(src.includes("const domIds = await collectAllIds(pageScroller(), { cap: Number(cfg.playlistItems) || 0, probe: true, escalate: true, onProgress:"),
     'page-dom накапливает ленивую ленту скроллом (кап = «сколько»), а не одним снимком');
  {
  ...
  ok(src.includes('const titles = (o.titles && o.titles.length) ? o.titles'),
     'job тянет заголовки каскадом: browse -> очередь страницы -> DOM-строки');
  {
  ...
  ok(src.includes('stable = 0;') && src.includes('if (isLoadingHint()) {'),
  ...
  ok(src.includes('const rowById = new Map(pageListRows().map((x) => [x.videoId, cleanMeta(x)]));'),
