...
eq(rel, 'QA Artist - Companion Test.m4a', 'имя собрано по шаблону «артист — название»');
...
`;
  ...
  if (tags) {
    ...
    // mutagen отдаёт trkn как [[номер, всего]]; «всего» companion пишет 0 — так и задумано,
    // YTM всё равно показывает только номер.
    ok(Array.isArray(tags.trkn) && Array.isArray(tags.trkn[0]) && tags.trkn[0][0] === 3,
      `номер трека: ${JSON.stringify(tags.trkn)}`);
...
group('companion: переопределение папки загрузки (?out=)');
{
  ...
  const inSub = path.join(target, 'QA - IntoSub.m4a');
  ok(fs.existsSync(inSub), `файл лёг в переопределённую папку: ${inSub}`);
  eq(j.dest, inSub, 'job.dest указывает именно туда');
  ok(!fs.existsSync(path.join(outDir, 'QA - IntoSub.m4a')), 'в папку по умолчанию (--out) ничего не попало');
...
group('портативная раскладка (dist/ytm-dl-win) и --check');
{
  ...
  if (!h2) { ok(false, 'companion из dist-папки поднялся\n' + tail()); proc2.kill('SIGKILL'); }
    ...
    ok(/^ytm-dl-win.music.Win - Portable.m4a$/.test(String(path.relative(path.join(root, 'dist'), job.dest)).split(path.sep).join('/')),
...
group('bot-чек: HTML вместо потока не должен становиться «треком»');
{
  ...
  // дорожку не оставляет без дела, а info.json-спасатель их подхватывает.
  {
    ...
    const scriptT = [
      ...
      'from mutagen.mp4 import MP4',
      'mp = MP4(p)',
      'out = {"written": sorted(k for k in w if not k.startswith("_")),',
    ...
    if (ht.tpe2 !== undefined) {
      ...
      eq(ht.trck, '5', 'номер дорожки в TRCK');
...
group('webm→контейнер по формату, обложка в opus, лестница клиентов в плейлисте');
{
  ...
  ok(/@version\s+0.6.17/.test(us), 'userscript bumped на 0.6.12');
...
group('0.5.2: proxy=none, ytm-dl.json, битые файлы не выдаём, /log, convert=');
{
  ...
  const F0 = parse(runPy(boot + [
    'import types, pathlib, tempfile',
    'tmp = pathlib.Path(tempfile.mkdtemp())',
    '(tmp / "x - y.m4a").write_bytes(b"old-file")',
  ...
  ok(F0.hit.includes('x - y.m4a'), 'файл в папке = «уже есть» (пропуск вместо (1))');
  ...
  const F2 = parse(runPy(boot + [
    'import types, pathlib, tempfile',
    'tmp = pathlib.Path(tempfile.mkdtemp())',
    '(tmp / "Art - Song.m4a").write_bytes(b"old-one")',
    '(tmp / "Art - Song2.m4a").write_bytes(b"old-two")',
    ...
    '    "kept": (tmp / "Art - Song.m4a").read_bytes() == b"old-one",',
...
// D) живой сервер: /log-зеркало с токеном, hello с build/convert, convert=mp3 для /media
{
  ...
  if (!h4) { ok(false, 'компаньон с --convert/--token поднялся\n' + b4.slice(-400)); }
    ...
    ok(h4.build === '0.6.17', `hello носит версию сборки: ${JSON.stringify(h4.build)}`);
    ...
    ok(lf.includes('build     : ytm-dl 0.6.17'), 'баннер печатает сборку - и она же видна в ytm-run.log (с --log-file stdout не в трубе)');
