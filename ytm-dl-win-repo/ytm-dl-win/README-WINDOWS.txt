...
"YouTube недоступен вообще" (РФ): прокси
    ...
    Как убедиться, что версия новая: заголовок панели сам показывает v0.6.17 (с
    этой версии цифра берётся из @version, а не захардкожена), а статус «companion
    ok (сборка 0.6.17)» - версию компаньона; при разнобое панель пишет «версии
    разошлись». А надёжнее всего: Tampermonkey -> свойства скрипта -> 0.6.17 (если там
