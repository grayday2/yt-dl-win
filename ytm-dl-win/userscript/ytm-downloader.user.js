...
(function (global) {
  ...
  function listRowAnchor() {
    ...
    // Значит найдены кандидаты, которые ПРИНИМАЮТ запись scrollTop, но картинки
    // на экране не двигают (overflow:hidden охотно хранит чужие числа). Единственный
    // честный свидетель - координата первой строки: сдвинулась - скроллер настоящий.
    try {
      const scope = mainListScope();
      if (!scope || !scope.querySelector) return null;
      const r = scope.querySelector('yt-lockup-view-model, ytmusic-responsive-list-item-renderer, ytmusic-two-row-item-renderer');
  ...
  function rowTexts(el) {
    ...
    try {
      ...
      if (fs.length) {
        ...
        byline = rest.find((x) => /[\u00b7\u2022]/.test(x)) || rest[rest.length - 1] || '';
  ...
  function collectAllIds(scroller, opt) {
    ...
    const seen = new Set();
    let stable = 0, lastCount = -1, noGrowth = 0, visStall = 0, lastVisTop = null;
    ...
    if (o.probe) {
      ...
      const pk = pickScroller(cand);
      mirrorLog(`скроллер: ${describeEl(pk || el)} (кандидатов ${cand.length}${pk ? '' : ', проба не сдвинула ни один'})`, 'info');
      if (pk) el = pk;
    }
    ...
    const collect = () => { for (const id of readIds()) { if (CAP && seen.size >= CAP) return; seen.add(id); } };
    ...
    const pass = () => {
      ...
      if (seen.size !== lastCount) {
        stable = 0; noGrowth = 0; visStall = 0; lastCount = seen.size; catched = false;
        if (o.onProgress) { try { o.onProgress(seen.size, tot); } catch (e) {} }
      ...
      if ((atBottom && stable >= stableFor) || noGrowth >= 14) {
        ...
        if (ci + 1 < cands.length && i + 4 < rounds && (!tot || seen.size < tot)) {
          ci++; elx = cands[ci]; catched = false; visStall = 0; lastVisTop = null;
          try { elx.scrollTop = 0; } catch (e) {}
          stable = 0; lastCount = -1; noGrowth = 0;
          mirrorLog(`тупик (${noGrowth >= 14 ? 'раунды без роста' : 'низ и тишина'}) - пробую скроллер #${ci + 1}: ${describeEl(elx)}`, 'info');
  ...
  async function queueFromPage(opt) {
    ...
    let ids = (o.ids && o.ids.length) ? o.ids
      : ((o.scroll === false) ? pageTrackIds() : await collectAllIds(pageScroller(), { cap: lim }));
    if (!ids.length) ids = await pageQueueAttempt();   // 0.6.6: панель «Следующее»
    if (lim > 0 && ids.length > lim) { ids = ids.slice(0, lim); mirrorLog(`беру первые ${lim} по полю «сколько»`, 'info'); }
    if (!ids.length) { mirrorLog(pageProbe(), 'info'); setStatus('на странице нет ни треклиста, ни очереди — открой альбом/плейлист или разверни панель «Следующее»', 'err'); return; }
    // 0.6.13: слияние по id, DOM-строки СВЕРХУ данных очереди: get_queue умеет
    // принести one-line byline, где альбом без артиста (бой: «ONLY IF I DIE...»
    // уехал в артисты), а в отрисованной строке - честное «Артист · Альбом».
    // Пустое - как и раньше добивает info.json компаньона.
    const cleanMeta = (x) => { const y = {}; if (x) for (const k in x) if (x[k]) y[k] = x[k]; return y; };
    const rowById = new Map(pageListRows().map((x) => [x.videoId, cleanMeta(x)]));
    const pqById = new Map(state.pageQueue.map((x) => [x.videoId, cleanMeta(x)]));
    const titles = (o.titles && o.titles.length) ? o.titles
      : ids.map((id) => Object.assign({ videoId: id }, pqById.get(id) || {}, rowById.get(id) || {}));
    ...
    for (let i = 0; i < fresh.length; i++) {
      ...
      const id = fresh[i];
      try {
        const t = titles.find((x) => x.videoId === id) || {};
  ...
  async function playThrough(opt) {
    ...
    let ids = (oo.ids && oo.ids.length) ? oo.ids
      : ((oo.scroll === false) ? pageTrackIds() : await collectAllIds(pageScroller(), { cap: lim, probe: true, escalate: true, onProgress: (nn, tt) => setStatus(`собрано ${nn}${tt ? ' из ' + tt : ''} - прокручиваю…`, 'run') }));
  ...
  async function downloadWholeList() {
    ...
    return job;
      ...
      const domIds = await collectAllIds(pageScroller(), { cap: Number(cfg.playlistItems) || 0, probe: true, escalate: true, onProgress: (nn, tt) => setStatus(`собрано ${nn}${tt ? ' из ' + tt : ''} - прокручиваю…`, 'run') });
      if (!domIds.length) mirrorLog('после прокрутки 0 строк - ' + pageProbe(), 'info');
      if (domIds.length) {
        mirrorLog(`browse закрыт, но на странице ${domIds.length} строк - ставлю их в очередь`, 'info');
        await queueFromPage({ ids: domIds, scroll: false });
        return { via: 'page-dom', count: domIds.length };
      }
      throw e;
    }
