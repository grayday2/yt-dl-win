...
API_VERSION = "1"
APP_VERSION = "0.6.3"   # версия СБОРКИ (не протокола): её печатает баннер, отдаёт /hello и selftest - по ней в логах видно, что реально установлено
...
class Job:
    ...
    def to_dict(self) -> dict:
        return _jsonable({
            ...
            "error": self.error, "dest": str(self.dest), "meta": self.meta,
...
TAG_MAP_MP4 = {
    ...
    "genre": "gnre", "date": "\xa9day", "track": "trkn", "albumartist": "aART",
...
def write_tags(path: Path, meta: dict, cover_url: str | None = None,
    ...
    data = {k: v for k, v in (meta or {}).items() if v not in (None, "", [])}
        ...
        try:
            if suf == ".mp3":
                ...
                for key, (frame, conv) in TAG_MAP_ID3.items():
                    if key in data:
                        tags.add(frame(encoding=3, text=str(conv(data[key]))))
...
def dest_for(job_meta: dict, out_dir: Path, suffix: str) -> Path:
    artist = safe_name(job_meta.get("artist") or "Unknown Artist", 60)
    title = safe_name(job_meta.get("title") or job_meta.get("videoId") or "track", 90)
    if job_meta.get("album") and job_meta.get("organize") == "album":
        folder = out_dir / safe_name(job_meta["album"], 60)
    else:
        folder = out_dir
    folder.mkdir(parents=True, exist_ok=True)
    base = f"{artist} - {title}" if job_meta.get("artist") else title
...
def _client_ladder(chosen):
    ...
    rest = [("web_safari", []), ("", []), ("tv", []), ("web_embedded", []),
            ("web_safari", ["--extractor-args", "youtube:formats=missing_pot"])]
    out, seen = [], set()
    for c, e in first + rest:
        k = (c, tuple(e))
        if k not in seen:
            seen.add(k); out.append((c, e))
    return out
...
def _playlist_client_variants(job):
    ...
    first = chosen + [c for c in ("web_safari",) if c not in chosen]
    ...
    return [{"player_client": first or ["web_safari"]},
            {"player_client": ["web_safari"], "formats": ["missing_pot"]},
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if u.path == "/log":
            ...
            txt = str(data.get("text") or "").strip()[:300].replace("\n", " ")
